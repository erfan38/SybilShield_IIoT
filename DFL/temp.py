    ...
    def generate_report(self):
        ...
        for peer_id in all_nodes:
            ...
            try:
                ...
                if missing:
                    print(f" Missing features in {peer_id}: {missing}")
                    continue

                grouped = df.groupby("node_id")

                for real_node_id, group_df in grouped:
                    sample_count = len(group_df)
                    if sample_count < 5:
                        print(f" Skipping {real_node_id} due to low sample count ({sample_count})")
                        continue
                    ...
                    with torch.no_grad():
                        ...
                        confidence = probs[:, 1].mean().item()
                        is_sybil_ratio = (predictions == 1).mean()
                        pred_counts = np.bincount(predictions, minlength=2)

                    print(f"[{real_node_id}]  Sybil ratio = {is_sybil_ratio:.2f}, Confidence = {confidence:.2f}, Predictions = {pred_counts.tolist()}")

                    # Adjusted thresholds for better sensitivity
                    if is_sybil_ratio > 0.5 and confidence > 0.5:
                        if confidence > 0.9:
                            severity = 8
                        ...
                        report["reports"].append({
                            "suspect_node": real_node_id,
                            "severity": severity,
                            "anomaly_score": round(confidence, 3),
                            "features": {
                                "sybil_confidence": round(confidence, 3),
                                "predicted_sybil_ratio": round(is_sybil_ratio, 2),
                                "sample_count": sample_count
                            }
                        })

            except Exception as e:
                print(f" Skipping {peer_id} due to error: {e}")

        # Save JSON report
        report_path = os.path.join(REPORT_DIR, f"{self.node_id}_report.json")
        with open(report_path, "w") as f:
            json.dump(report, f, indent=2)

        print(f"\n{self.node_id} report saved with {len(report['reports'])} suspect(s).")
