...
class CNNTrainer:
    def __init__(self, csv_path, epochs=200, lr=1.5e-2, batch_size=64,
        ...
        self.device = torch.device("cpu")              # keep it CPU-friendly
    ...
    def _prepare_data(path, batch, test_frac, val_frac, seed):
        df = pd.read_csv(path) 
        # X = df[["sent_total", "received_total", "protocol_diversity",
        #         "message_burstiness", "mqtt_ratio", "discovery_ratio",
        #         "protocol_entropy", "unique_peers",
        #         "dominant_protocol_ratio"]].values
        ...
        relative_val = val_frac / (1 - test_frac)      # e.g. 0.15 / 0.85 ≈ 0.176
...
if __name__ == "__main__":
    trainer = CNNTrainer(csv_path="trust_aware_iiot_dataset_balanced.csv",
                         epochs=150, lr=0.02)
    trainer.fit()
    trainer.test()
