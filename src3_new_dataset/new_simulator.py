...
# ]
# LOG_FILE = "new2_logs.jsonl"
# FINAL_CSV = "new2_dataset.csv"
# WINDOW_SIZE = 5
...
#         port = random.choice([30303, 30304, 30305])
#         pub_key = ''.join(random.choices(list('abc0123'), k=random.randint(45, 55)))
#         protocols = random.sample(["eth", "snap", "les", "custom"], k=random.randint(1, 3))
#     else:
#         ip = f"192.168.1.{random.randint(1, 255)}"
#         port = 30303
#         pub_key = ''.join(random.choices(list('abcdef0123456789'), k=random.randint(50, 65)))
#         protocols = ["eth", "snap"]
...
#         if not is_sybil and random.random() < 0.05:
#             self.behavior_type = random.choice(["spammer", "evasive"])
#             self.is_sybil = True
#         else:
...
#                 protocol = random.choice(["MQTT:PUBLISH"] * 5 + ["discovery:Ping", "libp2p:IWANT"])
#             elif self.behavior_type == "evasive":
#                 protocol = random.choice(["ENR:Request", "libp2p:PRUNE", "discovery:FindNode"])
#             else:
#                 protocol = random.choice(PROTOCOLS[:6])
#         else:
...
#     print("[+] Starting improved simulation...")
...
#             peers_sample = [n for n in net.enrs if n != node_id]
#             sample_peers = random.sample(peers_sample, 5)
#             peer_vecs = [enr_to_vector(net.enrs[p]) for p in sample_peers]
#             avg_enr_similarity_realistic = np.mean([cosine_similarity([enr_vec], [v])[0][0] for v in peer_vecs])
...
#                 "avg_enr_similarity_realistic": round(avg_enr_similarity_realistic, 4),
...
#     print(f"[✓] Final improved dataset saved to {FINAL_CSV}")
...
LOG_FILE = "new3_logs.jsonl"
FINAL_CSV = "new3_dataset.csv"
WINDOW_SIZE = 5
